...
import {Player, Position, Session, SessionData, Map, SessionOf, PlayerIndex, TileLookup} from "../codegen/index.sol";
import {TILE_SIZE, BASE_SPEED, TILE_HALF} from "../constants.sol";

contract SessionSystem is System {
    function createSession(bytes32 id, GameMode gameMode, TeamMode teamMode, bytes32 map) public {
        (uint32[4] memory spawnIndexes, bytes memory terrain) = Map.get(map);
        require(!LibUtils.bytesEq(terrain, hex""), "no map found");
        terrain = LibTilemap.fillWalls(spawnIndexes, terrain, 50, uint256(id));
    ...
    function leaveSession() public {
        ...
        Session.setPlayerBitmap(session, uint8(LibBitmap.reset(playerBitmap, playerIndex)));
